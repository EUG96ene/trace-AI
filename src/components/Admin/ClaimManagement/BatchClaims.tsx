...
export default function BatchClaims() {
  ...
  const fetchBatches = async () => {
    try {
      ...
      const batchesData = await Promise.all(response.data.map(async batch => {
        const claimsWithUserDetails = await Promise.all(batch.claims.map(async claim => {
  ...
  const columns: (ColumnDef<Batch> | ColumnDef<Claim>)[] = selectedBatch ? [
    // Claims columns
    {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem
                onClick={() => {
                  router.push(`/dashboard/client/claim/${claimID}`);
                }}
              >
                View Claim
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem>Merge Claim</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuLabel>Actions</DropdownMenuLabel>
            <DropdownMenuItem onClick={() => { fetchBatchById(row.original.id); }}>
              View Batch
            </DropdownMenuItem>
            <DropdownMenuSeparator />
          </DropdownMenuContent>
  ...
  const tableInstance = useReactTable({
    data: selectedBatch ? selectedBatch.claims : batches,
    columns,
    state: {
      sorting,
      columnFilters,
